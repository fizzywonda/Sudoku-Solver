...
class Polygon(GraphicsObject):
    def __init__(self, *points):
        ...
        self.lines = []
        self._setLines(self.points) #Added by Brian Bennett, September 2016
    ...
    def getLines(self):
        '''
            Added by Brian Bennett, September 2016
        '''
        return self.lines


    def intersects(self, pathSegment):
        '''
            Added by Brian Bennett, September 2016

            This method returns True if the pathSegment intersects any line.
            If it does, it is not a valid pathSegment. Note that this method
            returns False if the pathSegment is co-located with an existing line.

            Mathematically, this uses determinants from linear algebra.
        '''
        P1 = pathSegment.getP1().clone()
        P2 = pathSegment.getP2().clone()
        x1 = pathSegment.getP1().getX()
        x2 = pathSegment.getP2().getX()
        y1 = pathSegment.getP1().getY()
        y2 = pathSegment.getP2().getY()
        ...
        for line in self.lines:
            ...
            # If the lines are identical, it doesn't count as an intersection.
            if (str(P1) == str(P3) and str(P2) == str(P4)) or \
               (str(P1) == str(P4) and str(P2) == str(P3)):
                return False

        if (str(P1) in [str(p) for p in self.points] and \
            str(P2) in [str(p) for p in self.points]):
            intersect = True
        
        else:
            # Loop through all lines to find intersections
            for line in self.lines:
                x3 = line.getP1().getX()
                x4 = line.getP2().getX()
                y3 = line.getP1().getY()
                ...
                # if a and b are between 0 and 1 [exclusive], the lines intersect
                if ((a > 0 and a < 1) and (b > 0 and b < 1)):
                    intersect = True
                    break

        return intersect #or self.crosses(pathSegment)

    def _move(self, dx, dy):
        for p in self.points:
            p.move(dx, dy)

    def _setLines(self, points):
        '''
            Added by Brian Bennett, September 2016

            Create all the lines that make up the polygon
        '''
        for i in range(0,len(self.points)):
            if (i+1 >= len(self.points)):
                line = Line(self.points[i], self.points[0])
            else:
